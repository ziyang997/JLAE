...
# calculate precision@k, recall@k, NDCG@k, where k = 1,5,10,15,20
def user_precision_recall_ndcg(new_user_prediction, test):
    dcg_list = []

    # compute the number of true positive items at top k
    count_1, count_5, count_10, count_15, count_20 = 0, 0, 0, 0, 0
    for i in range(20):
    ...
    return np.array([precision_1, precision_5, precision_10, precision_15, precision_20]), \
           np.array([recall_1, recall_5, recall_10, recall_15, recall_20]), \
           np.array([ndcg_tmp_1, ndcg_tmp_5, ndcg_tmp_10, ndcg_tmp_15, ndcg_tmp_20])


# calculate the metrics of the result
def test_model_all(prediction, test_mask, train_mask):

    precision_1, precision_5, precision_10, precision_15, precision_20 = 0.0000, 0.0000, 0.0000, 0.0000, 0.0000
    recall_1, recall_5, recall_10, recall_15, recall_20 = 0.0000, 0.0000, 0.0000, 0.0000, 0.0000
    ndcg_1, ndcg_5, ndcg_10, ndcg_15, ndcg_20 = 0.0000, 0.0000, 0.0000, 0.0000, 0.0000
    precision = np.array([precision_1, precision_5, precision_10, precision_15, precision_20])
    recall = np.array([recall_1, recall_5, recall_10, recall_15, recall_20])
    ndcg = np.array([ndcg_1, ndcg_5, ndcg_10, ndcg_15, ndcg_20])
    ...
    for u in range(user_num):  # iterate each user
        ...
        top20_item_idx_no_train = np.argpartition(u_pred, -20)[-20:]
        top20 = (np.array([top20_item_idx_no_train, u_pred[top20_item_idx_no_train]])).T
        top20 = sorted(top20, key=itemgetter(1), reverse=True)

        # calculate the metrics
        if not len(u_test) == 0:
            precision_u, recall_u, ndcg_u = user_precision_recall_ndcg(top20, u_test)
    ...
    recall /= user_num
    ndcg /= user_num
    print('precision_1\t[%.7f],\t||\t precision_5\t[%.7f],\t||\t precision_10\t[%.7f],\t||\t precision_15\t[%.7f],\t||\t precision_20\t[%.7f]' \
          % (precision[0],
             precision[1],
             precision[2],
             precision[3],
             precision[4]))
    print('recall_1 \t[%.7f],\t||\t recall_5   \t[%.7f],\t||\t recall_10   \t[%.7f],\t||\t recall_15   \t[%.7f], \t||\t recall_20  \t[%.7f]' \
          % (recall[0], recall[1],
    ...
    f_measure_15 = 2 * (precision[3] * recall[3]) / (precision[3] + recall[3]) if not precision[3] + recall[
        3] == 0 else 0
    f_measure_20 = 2 * (precision[4] * recall[4]) / (precision[4] + recall[4]) if not precision[4] + recall[
        4] == 0 else 0
    print('f_measure_1\t[%.7f],\t||\t f_measure_5\t[%.7f],\t||\t f_measure_10\t[%.7f],\t||\t f_measure_15\t[%.7f], \t||\t f_measure_20\t[%.7f]' \
          % (f_measure_1,
             f_measure_5,
             f_measure_10,
             f_measure_15,
             f_measure_20))
    f_score = [f_measure_1, f_measure_5, f_measure_10, f_measure_15, f_measure_20]
    print('ndcg_1     \t[%.7f],\t||\t ndcg_5     \t[%.7f],\t||\t ndcg_10     \t[%.7f],\t||\t ndcg_15     \t[%.7f],\t||\t ndcg_20     \t[%.7f]' \
          % (ndcg[0],
             ndcg[1],
             ndcg[2],
             ndcg[3],
...
def test_all(sess, model):
    Decoder = sess.run(model.decoder, {model.users: list(range(model.num_users)),model.input_R_U: model.train_R, model.drop:1})
